...
import { Animal, Human, getFullName } from './model.js';
...
export async function printHumansAndAnimals() {
    const allP = await getFullName()
     const getInfo =  allP.map(async(el) => {
        const firstLast = [el.fname, el.lname].join(" ")
        const animal = await Animal.findByPk(el.humanId)
        return firstLast, animal
    })
